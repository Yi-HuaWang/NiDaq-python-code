...
counter.triggers.start_trigger.cfg_dig_edge_start_trig("PFI0", trigger_edge=cst.Edge.RISING)
...
Reader.timing.cfg_samp_clk_timing(s_freq, sample_mode=sampleMode_r, samps_per_chan=num_samples)
...
for ii in range(0,num_pusles):
    counter.wait_until_done(timeout=10.0)
    comulativeData[ii * num_samples:(ii + 1) * num_samples] = Reader.read(number_of_samples_per_channel=num_samples)
